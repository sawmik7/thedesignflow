...
export function Contact({ scrollToContact }: { scrollToContact: () => void }) {
    ...
    try {
      await emailjs.send(
        ...
        templateId,
        {
          ...
          subject: "Contact Form Submission (Section)"
        },
        publicKey
      );
      setSubmitted(true);
  ...
  return (
    <section
      ref={sectionRef}
      id="contact"
      className="relative py-24 md:py-32 px-6 md:px-12 w-full overflow-hidden transition-colors duration-700 bg-[var(--background)] text-[var(--foreground)] border-t border-neutral-300/20"
    >
      {/* Top divider */}
            ...
            {submitted ? (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <FormField label="Full Name" name="name" type="text" placeholder="John Doe" required />
                  <FormField label="Email Address" name="email" type="email" placeholder="john@company.com" required />
                </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <input
        ...
        className="w-full min-h-[48px] rounded-xl px-4 py-3 text-base font-medium outline-none transition-all duration-300 text-white"
        style={{
          background: "rgba(255,255,255,0.05)",
          border: "1px solid rgba(255,255,255,0.1)",
        }}
